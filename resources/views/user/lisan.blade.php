...
<body class="bg-gray-100">
    <div class="container mx-aut relative z-20">
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8 -mt-16">
            @if (session('success'))
                <script>
                    window.location.href = "{{ route('lisans.detail') }}";
                </script>
            @endif

            <form action="{{ route('lisans.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    <label for="date" class="block text-sm font-medium text-gray-700">Tanggal Kebutuhan</label>
                ...
                <div class="col-span-2">
                    ...
                    <textarea id="reason" name="reason" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"></textarea>
                    @error('reason')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="">
                    <button type="button"
                        class="bg-gray-700 text-white py-2 px-4 rounded-md hover:bg-gray-600"><a href="{{ route('home') }}">Kembali</a></button>
                    <button type="submit"
                        class="bg-red-700 text-white py-2 px-4 rounded-md hover:bg-red-600">Pesan sekarang</button>
                </div>
            </form>
        </div>
    </div>
